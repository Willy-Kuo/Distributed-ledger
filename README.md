...
本系統設計三個帳本節點，是為了模擬分散式帳本環境。三個節點可以同時展示多節點資料保存、節點間同步，以及鏈一致性檢查，比單一節點或雙節點更能體現分散式系統特性。若只有一個節點，就無法展示同步；若只有兩個節點，雖然可以看到資料複製，但較難完整呈現多節點比較與一致性檢查的概念，因此本專題採用三個節點作為示範架構。
